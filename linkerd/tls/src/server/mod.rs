...
pub fn empty_config() -> ServerConfig {
...
where
    P: ExtractParam<Option<Config<L>>, T> + Clone,
    N: Clone,
{
...
where
    ...
    NSvc::Future: Send,
{
    type Response = NSvc::Response;
    type Error = Error;
    type Future = Pin<Box<dyn Future<Output = Result<NSvc::Response, Error>> + Send + 'static>>;
